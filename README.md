...
- 恢复成功 1 秒后仅调用显示器休眠；键盘或鼠标唤醒后，会核对实际硬件亮度和 gamma，发现偏差时自动重写并复查。
...
选择“断开后息屏”时，守护程序会继续保留本次快照并监听 macOS 的显示器睡眠/唤醒事件。唤醒 4 秒后，它会读取内屏亮度、外屏 DDC 值和外屏 gamma 表；如果 MonitorControl 显示的百分比正确但实际画面仍偏暗，会重新应用快照并再次验证。只有验证成功后才删除快照，验证失败则每 3 秒继续重试。

快照和状态文件权限为 `0600`，只保存亮度值和临时显示器标识，不保存显示器名称、序列号、UU 账号、远端设备 ID 或网络地址。不启用息屏时，恢复成功即删除快照；启用息屏时，唤醒后验证成功才删除。
...
| `UURBG_POST_WAKE_DELAY` | `4.0` | 收到显示器唤醒事件后，等待硬件稳定再校验的秒数。 |
| `UURBG_POST_WAKE_RETRY` | `3.0` | 唤醒后校验或修复未成功时的重试间隔。 |
